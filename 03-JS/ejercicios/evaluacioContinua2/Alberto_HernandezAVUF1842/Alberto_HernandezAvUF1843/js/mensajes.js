const elegirAcierto = function(palabra){
  ...
  let numRandom = Math.floor(Math.random() * msg.length);
  const enviarAcierto = msg[numRandom]
...
const elegirError  = function(palabra){ 
  ...
  let numRandom = Math.floor(Math.random() * msgError.length);
  const enviarError = msgError[numRandom]
  return enviarError;
}
